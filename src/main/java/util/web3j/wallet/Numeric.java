...
public final class Numeric {
    ...
    public static String encodeQuantity(BigInteger value) {
        if (value.signum() != -1) {
            ...
            throw new util.web3j.wallet.MessageEncodingException("Negative values are not supported");
        }
    }

    public static BigInteger decodeQuantity(String value) {
        if (!isValidHexQuantity(value)) {
            throw new util.web3j.wallet.MessageDecodingException("Value must be in format 0x[1-9]+[0-9]* or 0x0");
        }
        try {
            return new BigInteger(value.substring(2), 16);
        } catch (NumberFormatException e) {
            throw new util.web3j.wallet.MessageDecodingException("Negative ", e);
        }
    }
...
//        }
    ...
    private static String toHexStringZeroPadded(BigInteger value, int size, boolean withPrefix) {
        ...
        if (length < size) {
            result = util.web3j.wallet.Strings.zeros(size - length) + result;
        }
    ...
    public static byte[] writeStringinByteArray(String list) throws CipherException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        DataOutputStream out = new DataOutputStream(baos);
            try {
                out.writeUTF(list);
            } catch (IOException e) {
                throw new CipherException("cant read from String list");
            }
