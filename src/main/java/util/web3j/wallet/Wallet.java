...
public class Wallet {
    ...
    public static MnemonicModel createMnemonic(String password, String code, int n, int p)
            throws CipherException {
        ...
        byte[] mnemonicBytes = Numeric.writeStringinByteArray(code);

        byte[] cipherText = performCipherOperation(
                Cipher.ENCRYPT_MODE, iv, encryptKey, mnemonicBytes);

        byte[] mac = generateMac(derivedKey, cipherText);
        return createMnemonicModel(cipherText, iv, salt, mac, n, p);
    }
    ...
    public static MnemonicModel createMnemonic(String password, String code)
            throws CipherException {
        return createMnemonic(password, code, N_LIGHT, P_LIGHT);
    }
    ...
    private static MnemonicModel createMnemonicModel(byte[] cipherText, byte[] iv, byte[] salt, byte[] mac,
            int n, int p) {

        MnemonicModel mnemonicModel = new MnemonicModel();

        MnemonicModel.Crypto crypto = new MnemonicModel.Crypto();
        crypto.setCipher(CIPHER);
        crypto.setCiphertext(Numeric.toHexStringNoPrefix(cipherText));
        mnemonicModel.setCrypto(crypto);

        MnemonicModel.CipherParams cipherParams = new MnemonicModel.CipherParams();
        cipherParams.setIv(Numeric.toHexStringNoPrefix(iv));
        crypto.setCipherparams(cipherParams);

        crypto.setKdf(SCRYPT);
        MnemonicModel.ScryptKdfParams kdfParams = new MnemonicModel.ScryptKdfParams();
        kdfParams.setDklen(DKLEN);
        kdfParams.setN(n);
        kdfParams.setP(p);
        kdfParams.setR(R);
    ...
    public static String decryptMnemonicChpiher(String password, MnemonicModel mnemonicModel)
            throws CipherException {

        MnemonicModel.Crypto crypto = mnemonicModel.getCrypto();
        ...
        MnemonicModel.KdfParams kdfParams = crypto.getKdfparams();
        if (kdfParams instanceof MnemonicModel.ScryptKdfParams) {
            MnemonicModel.ScryptKdfParams scryptKdfParams =
                    (MnemonicModel.ScryptKdfParams) crypto.getKdfparams();
            int dklen = scryptKdfParams.getDklen();
            int n = scryptKdfParams.getN();
            int p = scryptKdfParams.getP();
            int r = scryptKdfParams.getR();
            byte[] salt = Numeric.hexStringToByteArray(scryptKdfParams.getSalt());
            derivedKey = generateDerivedScryptKey(password.getBytes(UTF_8), salt, n, r, p, dklen);
        } else if (kdfParams instanceof MnemonicModel.Aes128CtrKdfParams) {
            MnemonicModel.Aes128CtrKdfParams aes128CtrKdfParams =
                    (MnemonicModel.Aes128CtrKdfParams) crypto.getKdfparams();
            int c = aes128CtrKdfParams.getC();
            String prf = aes128CtrKdfParams.getPrf();
            byte[] salt = Numeric.hexStringToByteArray(aes128CtrKdfParams.getSalt());
        ...
        byte[] encryptKey = Arrays.copyOfRange(derivedKey, 0, 16);
        byte[] mnemonicBytes = performCipherOperation(Cipher.DECRYPT_MODE, iv, encryptKey, cipherText);
        byte[] tempBytes = new byte[mnemonicBytes.length-2];
        System.arraycopy(mnemonicBytes,2,tempBytes,0,tempBytes.length);
